...
class FLAIRModel(torch.nn.Module):
    ...
    def load_from_pretrained(self, weights_path=None):
        ...
        # --- robust checkpoint loading (compat with transformers 'position_ids' buffer) ---
        state = torch.load(weights_path, map_location="cpu")

        # Some checkpoints wrap the actual state_dict
        ...
        # Drop position_ids buffers if present (safe to ignore)
        for kk in list(state.keys()):
            if kk.endswith("embeddings.position_ids") or ".embeddings.position_ids" in kk:
                state.pop(kk, None)

        # Load strictly after cleanup
    ...
    def forward_multilabel(self, image, text):
        # self.eval()
